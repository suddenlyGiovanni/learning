...
following [@MPJ](https://github.com/mpj) video on [Composition over Inheritance](https://youtu.be/wfMtDGfHWpA)
...
const driver = (state: { position: number }) => ({
  drive(speed: number): number {
    return (state.position = state.position + speed)
  },
})
 ...
 * cleaningRobot = driver + cleaner
 */
const makeCleaningRobot = (name: string, position: number = 0) => {
  const state = {
    name,
    position,
  }
 ...
 * killerRobot = driver + killer
 */
const makeKillerRobot = (name: string, position: number = 0) => {
  const state = {
    name,
    position,
  }
