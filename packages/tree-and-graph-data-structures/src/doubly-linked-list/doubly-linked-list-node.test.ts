...
import type { IDoublyLinkedListNode } from './doubly-linked-list.interface'

describe('linked List `Node` class', () => {
  const setProperty = <T>(node: IDoublyLinkedListNode<T>) => (
    data: T
  ...
  const concatNodes = <T>(
    node1: IDoublyLinkedListNode<T>,
    node2: IDoublyLinkedListNode<T>
  ): IDoublyLinkedListNode<T> => {
    node1.next = node2
    return node1
  }
  ...
  it('should be initialize with any data type', () => {
    ...
    // eslint-disable-next-line jest/prefer-to-be-null
    expect(nullNode.element).toBe(null)
    ...
    expect(undefinedNode.element).toBe(undefined)
  })
