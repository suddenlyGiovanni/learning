...
const ElaborateInput: FC<{
  hasError: boolean
  placeholder: string
  value: string
  update: Dispatch<SetStateAction<string>>
}> = forwardRef(({ hasError, placeholder, value, update }, ref) => {
...
const ImperativeHandleComponent: FC = () => {
  const [city, setCity] = useState('Seattle')
  const [state, setState] = useState('WA')
  const [error, setError] = useState('')
  const cityEl = useRef<HTMLInputElement | undefined>()
  const stateEl = useRef<HTMLInputElement | undefined>()
